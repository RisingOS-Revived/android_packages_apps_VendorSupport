...
class SystemSettingSwitchPreference : SelfRemovingSwitchPreference {
    ...
    override fun isPersisted(): Boolean {
        return if (isLineageSettings) {
            LineageSettings.System.getString(context.contentResolver, key) != null
        } else {
    ...
    override fun putBoolean(key: String, value: Boolean) {
        if (isLineageSettings) {
            LineageSettings.System.putInt(context.contentResolver, key, if (value) 1 else 0)
        } else {
            Settings.System.putIntForUser(context.contentResolver, key, if (value) 1 else 0, UserHandle.USER_CURRENT)
        }
    }

    override fun getBoolean(key: String, defaultValue: Boolean): Boolean {
        return if (isLineageSettings) {
            LineageSettings.System.getInt(context.contentResolver, key, if (defaultValue) 1 else 0) != 0
        } else {
            Settings.System.getIntForUser(context.contentResolver, key, if (defaultValue) 1 else 0, UserHandle.USER_CURRENT) != 0
        }
    }
}
